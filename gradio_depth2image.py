...
def process(input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
    with torch.no_grad():
        ...
        ksizes = [7, 13, 21, 31, 45, 65]
        detected_map_blur = [detected_map] + [cv2.GaussianBlur(detected_map, (k, k), 0) for k in ksizes]
    ...
    with open(f'result_imgs/depth{fileno}.txt', 'w') as f:
        f.write(f'GaussianBlur: {ksizes}\n')
        for param in [prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta]:
            f.write(f'{param}\n')
    cv2.imwrite(f'result_imgs/depth{fileno}.jpg', cv2.cvtColor(results_img, cv2.COLOR_RGB2BGR))
    return results
